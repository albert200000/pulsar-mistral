...
import MistralApi from "./mistral-api.js";
...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(
      atom.commands.add(".mistral-pane-item", {
  ...
  async add() {
    ...
    const message = `<span class="hidden-filepath">${filePath ? filePath : ""}</span><span class="hidden-selection">${startRow ? `${startRow} ${startColumn} ${endRow} ${endColumn}` : ""}</span>
\`\`\`${language}
${text}
\`\`\`
    `;
  ...
  async fetchModels() {
    if (!this.mistralView) return [];

    const res = await MistralApi.modelsList();
