...
export class CodeBlockHandler {
  ...
  createApplyButton(htmlElem, editor) {
    const button = document.createElement("button");
    ...
    button.addEventListener(
      ...
      async (event) => {
        ...
        const filepath = htmlElem.querySelector(".hidden-filepath").textContent;
        const selectionElem = htmlElem.querySelector(".hidden-selection");
  ...
  handleCodeBlocks(htmlElem, isUser = false) {
    ...
    codeBlocks.forEach((codeBlock) => {
      ...
      if (!isUser) {
        const applyButton = this.createApplyButton(htmlElem, editor);
