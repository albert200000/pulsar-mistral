...
export default class MistralView {
  ...
  constructor(serializedState) {
    ...
    if (serializedState && serializedState.messages) {
      this.renderMessages(serializedState.messages);
    }
  ...
  async initConversations() {
    const res = await betaConversationsList(this.mistral);
    ...
    if (conversations.length) {
      ...
      select.addEventListener("change", async (event) => {
        ...
        const res = await betaConversationsGetMessages(this.mistral, {
          conversationId,
        });
    ...
    if (userMessage) {
      ...
      try {
        ...
        if (this.conversationId) {
          response = await betaConversationsAppendStream(this.mistral, {
            conversationId: this.conversationId,
            conversationAppendStreamRequest: commonFields,
          });
        } else {
          response = await betaConversationsStartStream(this.mistral, {
        ...
        for await (const event of result) {
          this.conversationId =
            event.data?.conversationId || this.conversationId;
